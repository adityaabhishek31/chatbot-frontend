...
import getJwt from "../utils/JwtFetcher";
...
const UserChatBot = () => {
  ...
  const handleAvailableDates = async (station) => {
    ...
    setOptions([]);
    try {
      const response = await axios.post("http://localhost:8082/api/chat/available-dates", {
        station,
      });
  ...
  const handleAvailableTimes = async (date) => {
    ...
    setOptions([]);
    try {
      const response = await axios.post("http://localhost:8082/api/chat/available-times", {
  ...
  const handleBookTimeslot = async (time) => {
    ...
    setOptions([]);
    try {
      const response = await axios.post("http://localhost:8082/api/chat/book-timeslot", {
        station: selectedStation,
        date: selectedDate,
        time,
      });
